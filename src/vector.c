...
Vector_t *Vector_Create(uint64_t initial_size, uint32_t alloc_step) {
  ...
  vector->items = myMalloc(initial_size * sizeof (short));
...
bool Vector_At(const Vector_t *const vector, uint64_t position, short *const value) {
...
void Vector_Append(Vector_t *const vector, short value) {
  if(vector == NULL){
      return;
  }

  if(vector->free_cells == 0){
      short *p = myRealloc(vector->items, (vector->size + vector->alloc_step) * sizeof (short));
...
bool Vector_Contains(const Vector_t *const vector, short value) {
    if(vector == NULL){
        return false;
    }
...
uint64_t Vector_IndexOf(const Vector_t *const vector, short value, uint64_t from) {
    if(vector == NULL){
        return -1;
    }
...
void Vector_Fill(const Vector_t *const vector, short value, uint64_t start_position, uint64_t end_position) {
    if(vector == NULL){
        return;
    }
